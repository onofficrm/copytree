...
export const Header: React.FC<HeaderProps> = ({
  ...
  // GnuBoard head.php snippet for developers
  const gnuBoardSnippet = `<!-- [그누보드 5 / Cursor IDE 연동용 GNB 마크업 예시: theme/basic/head.php] -->
<header id="site-header" class="site-header">
  <!-- 상단 안심/신뢰 바 -->
  <div class="top-notice-bar bg-dark-navy text-xs py-1.5 px-4 text-slate-300 border-b border-navy-border">
    <div class="container flex justify-between items-center">
      <div class="trust-badge flex items-center gap-2">
        <span class="text-gold font-bold">100% 비밀보장</span> | <span>전국 법원 비대면 전자소송</span>
      </div>
      <a href="tel:1800-0000" class="tel-link text-gold font-bold">상담전화: 1800-0000</a>
    </div>
  </div>

  <!-- 메인 헤더 네비게이션 (높이: 68px) -->
  <div class="gnb-main-wrap bg-primary-navy border-b border-navy-border">
    <div class="container flex items-center justify-between h-[68px]">
      <!-- 로고 -->
      <a href="<?php echo G5_URL; ?>" class="site-logo flex items-center gap-2.5">
        <div class="logo-icon bg-slate-800 text-gold p-2 rounded-xl">창원</div>
        <div class="logo-text">
          <span class="font-bold text-white text-lg">창원개인회생</span>
          <span class="badge text-xs text-gold ml-1">안심지원</span>
        </div>
      </a>

      <!-- 데스크탑 GNB -->
      <nav class="desktop-gnb hidden lg:flex items-center gap-2">
        <!-- 개인회생 드롭다운/메가메뉴 -->
        <div class="menu-item relative group">
          <a href="<?php echo G5_BBS_URL; ?>/page.php?pid=rehab" class="menu-link">개인회생</a>
          <div class="dropdown-menu hidden group-hover:block absolute top-full left-0 w-64 bg-[#1A2234] p-3 rounded-xl shadow-xl">
            <a href="?pid=rehab&tab=overview">개인회생이란</a>
            <a href="?pid=rehab&tab=qualification">개인회생 자격</a>
            <a href="?pid=rehab&tab=process">개인회생 절차</a>
            <a href="?pid=rehab&tab=docs">준비서류</a>
            <a href="<?php echo G5_BBS_URL; ?>/write.php?bo_table=counsel">개인회생 상담</a>
          </div>
        </div>
        <!-- 개인파산 드롭다운 -->
        <div class="menu-item relative group">
          <a href="<?php echo G5_BBS_URL; ?>/page.php?pid=bankruptcy" class="menu-link">개인파산</a>
        </div>
        <!-- 경험담 게시판 (/story) -->
        <a href="<?php echo G5_BBS_URL; ?>/board.php?bo_table=story" class="menu-link">신용회복 경험담</a>
        <!-- 뉴스/정보 게시판 (/news) -->
        <a href="<?php echo G5_BBS_URL; ?>/board.php?bo_table=news" class="menu-link">뉴스/정보</a>
        <!-- 자가진단 -->
        <a href="<?php echo G5_BBS_URL; ?>/page.php?pid=diagnosis" class="menu-link-diagnosis">자가진단</a>
      </nav>

      <!-- 우측 CTA -->
      <div class="header-cta hidden lg:flex items-center gap-4">
        <a href="tel:1800-0000" class="tel-cta">1800-0000</a>
        <a href="<?php echo G5_BBS_URL; ?>/write.php?bo_table=counsel" class="btn-gold-counsel">상담신청</a>
      </div>
    </div>
  </div>
</header>`;
  ...
  const copySnippet = () => {
    navigator.clipboard.writeText(gnuBoardSnippet);
    setCopiedCode(true);
  ...
  return (
    <>
      {/* 1. Top Meta System Bar (Developer & Prototype Toolbar) */}
      <div
        id="dev-prototype-bar"
        className="bg-[#0b101b] text-gray-300 text-xs py-1.5 px-4 border-b border-slate-800/80 select-none"
      >
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
          {/* View Mode Navigation Tabs */}
          <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
            <span className="text-slate-400 font-medium mr-1 text-[11px] hidden md:inline">
              뷰어 모드:
            </span>
            <button
              id="btn-mode-live"
              ...
              className={`px-2.5 py-1 rounded transition-colors flex items-center gap-1.5 text-xs ${
                ['home', 'rehab', 'bankruptcy', 'story', 'news', 'diagnosis', 'consultation'].includes(
                  currentPage
                )
                  ? 'bg-[#D7AE66] text-[#151C2C] font-bold shadow-xs'
            ...
            <button
              id="btn-mode-sitemap"
              onClick={() => handleNavClick('sitemap-ux')}
              className={`px-2.5 py-1 rounded transition-colors flex items-center gap-1.5 text-xs ${
                currentPage === 'sitemap-ux'
                  ? 'bg-[#D7AE66] text-[#151C2C] font-bold shadow-xs'
                  : 'text-gray-300 hover:text-white hover:bg-slate-800'
              }`}
            >
            ...
            <button
              id="btn-mode-cursor"
              onClick={() => handleNavClick('cursor-guide')}
              className={`px-2.5 py-1 rounded transition-colors flex items-center gap-1.5 text-xs ${
                currentPage === 'cursor-guide'
                  ? 'bg-[#D7AE66] text-[#151C2C] font-bold shadow-xs'
                  : 'text-gray-300 hover:text-white hover:bg-slate-800'
              }`}
            >
              <FileCode className="w-3.5 h-3.5" />
              <span>그누보드 연동 가이드</span>
            </button>

            {/* Quick GnuBoard HTML Code Modal Trigger */}
            <button
              id="btn-gnuboard-header-code"
              onClick={() => setShowCodeModal(true)}
              className="ml-1 px-2 py-0.5 rounded bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-[#D7AE66] border border-slate-700 text-[11px] flex items-center gap-1 transition-colors"
              title="헤더 HTML/그누보드 마크업 확인"
            >
              <Code2 className="w-3 h-3 text-[#D7AE66]" />
              <span>헤더 PHP/HTML 코드</span>
            </button>
          </div>
          ...
          <div className="flex items-center gap-1 bg-slate-900/90 p-0.5 rounded border border-slate-700/60">
            <button
              ...
              className={`p-1.5 rounded transition-colors flex items-center gap-1 text-[11px] ${
                deviceMode === 'desktop'
                  ? 'bg-slate-700 text-[#D7AE66] font-bold'
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            >
              ...
              <span className="text-xs font-semibold">1800-0000</span>
            </a>
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="pt-3 border-t border-[#222E46] flex flex-col gap-2 text-xs text-slate-400">
              <div className="flex items-center justify-between">
                <span>전화문의</span>
                <a href="tel:1800-0000" className="text-[#D7AE66] font-bold">
                  1800-0000
                </a>
              </div>
      ...
      {/* 5. GnuBoard & Cursor IDE Developers Code Modal */}
      {showCodeModal && (
        <div
        ...
        >
          <div
            className="bg-[#151C2C] text-slate-200 border border-[#2D3C5A] rounded-2xl max-w-3xl w-full p-6 space-y-4 shadow-2xl animate-fadeIn"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between border-b border-[#25324C] pb-3">
              <div className="flex items-center gap-2">
                <Code2 className="w-5 h-5 text-[#D7AE66]" />
                <h3 className="font-extrabold text-white text-base">
                  그누보드 5 / Cursor IDE 연동용 Header 마크업 명세
                </h3>
              </div>
              <button
                onClick={() => setShowCodeModal(false)}
            ...
            <p className="text-xs text-slate-300">
              개발자가 Cursor IDE에서 그대로 복사하여 그누보드 테마(
              <code className="text-[#D7AE66] font-mono">theme/basic/head.php</code>)에 바로 넣을 수 있도록
              시맨틱 태그 및 그누보드 URL 치환 상수로 정돈된 코드입니다.
            </p>

            <div className="relative">
              <pre className="bg-[#0B0F17] p-4 rounded-xl text-xs font-mono text-slate-300 overflow-x-auto max-h-80 border border-slate-800 leading-relaxed">
                {gnuBoardSnippet}
              </pre>
              <button
                onClick={copySnippet}
                className="absolute top-3 right-3 px-3 py-1.5 rounded-lg bg-[#D7AE66] text-[#151C2C] text-xs font-bold flex items-center gap-1.5 hover:bg-[#c59c53] shadow-md transition-all"
              >
            ...
            <div className="flex items-center justify-between pt-2 text-xs text-slate-400">
              <span>• 헤더 높이 68px, 완벽한 반응형 브레이크포인트 (1024px 데스크탑 / 이하 모바일)</span>
              <button
                onClick={() => setShowCodeModal(false)}
